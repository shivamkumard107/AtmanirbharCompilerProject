...
typedef enum {
    nd_Ident, nd_String, nd_Integer, nd_Sequence, nd_If, nd_indexrtc, nd_indexrts, nd_indexrti, nd_While,
...
#define newvar(name, type)  type *name = NULL;          \
                            int _curr_ ## name ## _index = 0;  \
                            int _curr_ ## name ## _max = 0
 
#define allocate(name)      do {if (_curr_ ## name ## _index >= _curr_ ## name ## _max) \
                                name = realloc(name, (_curr_ ## name ## _max += 32) * sizeof(name[0]));} while (0)

#define append(name, x)  do {allocate(name); name[_curr_ ## name ## _index++] = x;} while (0)
#define length(name)        _curr_ ## name ## _index
#define add(name)        do {allocate(name); _curr_ ## name ## _index++;} while (0)
 
FILE *source_fp, *dest_fp;
static int here;
newvar(object, code);
newvar(globals, const char *);
newvar(string_indexool, const char *);
...
int fetch_var_offset(const char *id) {
    for (int i = 0; i < length(globals); ++i) {
        if (strcmp(id, globals[i]) == 0)
            return i;
    }
    add(globals);
    int n = length(globals) - 1;
    globals[n] = strdup(id);
    return n;
}
 
int fetch_string_offset(const char *st) {
    for (int i = 0; i < length(string_indexool); ++i) {
        if (strcmp(st, string_indexool[i]) == 0)
            return i;
    }
    add(string_indexool);
    int n = length(string_indexool) - 1;
    string_indexool[n] = strdup(st);
    return n;
}
...
void list_code() {
    fprintf(dest_fp, "Datasize: %d Strings: %d\n", length(globals), length(string_indexool));
    for (int i = 0; i < length(string_indexool); ++i)
        fprintf(dest_fp, "%s\n", string_indexool[i]);
